...
#define FOCALTECH_ESD_PROTECTION_INFO                                          \
	"File Version of  focaltech_esd_protection.c:  V1.1.0 2016-03-24"
...
int fts_esd_protection_init(void)
{
	...
	thread_esd_protection = kthread_run(fts_esd_protection_timeout, 0,
					    "focal_esd_protection");
...
static int fts_esd_protection_timeout(void *unused)
{
	...
	do {
		wait_event_interruptible_timeout(esd_protection_waiter, 0,
						 uljiffies);
		...
		do_gettimeofday(&tv);
		iDeltaTime =
			(tv.tv_sec - g_last_comm_time.tv_sec) * MSEC_PER_SEC +
			(tv.tv_usec - g_last_comm_time.tv_usec) / 1000;
